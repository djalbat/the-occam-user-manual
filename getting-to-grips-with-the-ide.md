...
There are also three buttons for creating, joinning and leaving sessions, as well as a button to copy the session key to the clipboard.
...
The recommendation is that you do this and then pass the session key to other sessions users by email or whatever means.
They can paste the session key into the session key input and then clickn the join session button to join the session.
Anyone can leave a sesssion at any time, including its creator.
In fact the session will persist on the server for half a minute or so even when everyone has left it and can be re-joined during this time.
